...
def depthFirstSearch(gameState):
    """Implement depthFirstSearch approach, data structure: stack"""
...
def breadthFirstSearch(gameState):
    """Implement breadthFirstSearch approach, data structure: queue"""
    beginBox = PosOfBoxes(gameState)
    beginPlayer = PosOfPlayer(gameState)
    # Start state
    ...
    while frontier:
        ...
        # Get the actions taken to reach this node
        ...
        if node[-1] not in exploredSet:
            ...
            for action in legalActions(node[-1][0], node[-1][1]):
                ...
                if isFailed(newPosBox):
                    continue  
                # Add the new state to the frontier along with the actions taken to reach it
                frontier.append(node + [(newPosPlayer, newPosBox)])
                actions.append(node_action + [action[-1]])
    return temp
